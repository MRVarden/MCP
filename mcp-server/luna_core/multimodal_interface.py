...
if __name__ == "__main__":
    ...
    async def test_multimodal_interface():
        """Test de l'interface multimodale"""

        # Initialiser l'interface
        interface = LunaMultimodalInterface()
        ...
        profile = await interface.create_user_profile(
            ...
            name="Varden",
            preferences={
                "language": {"primary": "french", "technical": True},
                "accessibility": {"high_contrast": False}
            }
        )
        print(f"  Profile created: {profile.name} (φ={profile.phi_affinity:.3f})")

        # Tester différentes modalités
        test_inputs = [
            ("Bonjour Luna, comment vas-tu?", None),
            ("J'ai besoin d'aide urgente!", CommunicationModality.TEXT),
            ({"command": "analyze", "data": [1, 2, 3]}, CommunicationModality.VISUAL)
        ]

        for input_data, modality in test_inputs:
            ...
            # Afficher la réponse
            print(f"  Primary modality: {response.primary_modality.name}")
            print(f"  φ alignment: {response.phi_alignment:.3f}")
            print(f"  Emotional tone: {list(response.emotional_tone.keys())[:3]}")

            # Rendre en texte
            text_output = interface.render_message(response, format="text")
            print(f"  Response: {text_output[:100]}...")

        # Afficher les métriques
        metrics = interface.get_interface_metrics()
        print(f"\n📊 Interface Metrics:")
        print(f"  Messages processed: {metrics['usage']['messages_sent']}")
        print(f"  Modalities used: {list(metrics['modality_distribution'].keys())}")
        print(f"  Adaptation success: {metrics['adaptation_success']:.2f}")
